...
(function (global) {
  class TabVaultLinksManager {
    ...
    static async deleteLink(linkId) {
      ...
      this._emitChange();
      if (typeof TabVaultSyncEngine !== 'undefined') {
        TabVaultSyncEngine.recordLinkDelete(linkId);
      }
    }
    ...
    static async clearLinks(onlyRead = false) {
      const links = await this.getLinks();
      const toDelete = onlyRead ? links.filter((l) => l.isRead) : links;
      const remaining = onlyRead ? links.filter((l) => !l.isRead) : [];
      await TabVaultAPI.storage.local.set({ savedLinks: remaining });
      this._emitChange();
      if (typeof TabVaultSyncEngine !== 'undefined') {
        toDelete.forEach((l) => TabVaultSyncEngine.recordLinkDelete(l.id));
      }
    }
