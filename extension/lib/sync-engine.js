  ...
  class TabVaultSyncEngine {
    ...
    async syncNow() {
      ...
      try {
        ...
        // Save updated merged state for sessions
        ...
        // 2. Execute delta sync for Saved Reading Links
        const linksData = await TabVaultAPI.storage.local.get([
          'savedLinks',
          'tabvault_last_sync_links',
          'tabvault_pending_link_deletions',
        ]);
        const localLinks = linksData.savedLinks || [];
        ...
        const clientLinkChanges = localLinks.map((l) => ({
          ...
          pageUrl: l.pageUrl || '',
          savedAt: l.savedAt || Date.now(),
          isRead: !!l.isRead,
          readAt: l.readAt || null,
          tags: l.tags || [],
          clientUpdatedAt: l.updatedAt || l.savedAt || Date.now(),
          deletedAt: null,
        }));

        pendingLinkDeletions.forEach((del) => {
          clientLinkChanges.push({
            id: del.id,
            url: 'https://deleted.link',
            title: 'Deleted Link',
            hostname: 'link',
            savedAt: del.deletedAt,
            isRead: true,
            tags: [],
            clientUpdatedAt: del.deletedAt,
            deletedAt: del.deletedAt,
        ...
        try {
          const linksRes = await TabVaultApiClient.deltaSyncLinks(lastSyncLinks, clientLinkChanges);
          const { serverChanges: serverLinkChanges, newSyncTimestamp: newLinksTimestamp } = linksRes.data;

          let mergedLinks = [...localLinks];
          if (serverLinkChanges && serverLinkChanges.length > 0) {
            serverLinkChanges.forEach((remoteLink) => {
              if (remoteLink.deletedAt) {
                mergedLinks = mergedLinks.filter((l) => l.id !== remoteLink.id);
              } else {
                const existingIdx = mergedLinks.findIndex((l) => l.id === remoteLink.id);
                if (existingIdx !== -1) {
                  const localUpdated = mergedLinks[existingIdx].updatedAt || mergedLinks[existingIdx].savedAt || 0;
                  const remoteUpdated = remoteLink.clientUpdatedAt || remoteLink.serverUpdatedAt || 0;
                  if (localUpdated < remoteUpdated) {
                  ...
                  mergedLinks.unshift(remoteLink);
                }
              }
            });
          }
          ...
          await TabVaultAPI.storage.local.set({
            ...
            tabvault_last_sync_links: newLinksTimestamp,
            tabvault_pending_link_deletions: [],
          });
          ...
          console.warn('TabVault Links Sync Notice:', linkSyncErr.message);
        }
    ...
    /**
     * Record a saved link deletion for sync propagation
     */
    async recordLinkDelete(linkId) {
      const data = await TabVaultAPI.storage.local.get(['tabvault_pending_link_deletions']);
      const pending = data.tabvault_pending_link_deletions || [];
      ...
      await TabVaultAPI.storage.local.set({ tabvault_pending_link_deletions: pending });
      this.onLocalDataChanged();
    }
