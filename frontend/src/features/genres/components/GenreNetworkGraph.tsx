...
interface GenreNetworkGraphProps {
    data: GenreWithBooks[]
}

export function GenreNetworkGraph({ data }: GenreNetworkGraphProps) {
    ...
    useEffect(() => {
        ...
        // Create Nodes
        data.forEach(genreWithBooks => {
            const genreIndex = nodesList.length
            nodesList.push({
                id: `genre-${genreWithBooks.genre.id}`,
                type: 'genre',
                label: genreWithBooks.genre.name,
                data: genreWithBooks.genre,
            ...
            genreWithBooks.books.forEach(book => {
                ...
                linksList.push({ source: genreIndex, target: bookIndex })
            })
        })
    ...
    return (
        <div className="w-full bg-surface/20 rounded-3xl border border-white/5 overflow-hidden backdrop-blur-xl group">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <div>
                    <h3 className="text-lg font-semibold text-content-primary">Genre & Book Categorization Map</h3>
                    <p className="text-sm text-content-muted">Explore genres and their books in an interactive network</p>
                </div>
                <div className="flex gap-4 text-xs">
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-accent shadow-[0_0_8px_rgba(226,161,140,0.4)]" />
                        <span className="text-content-secondary">Genre</span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                >
                    ...
                    {nodes.map((node, i) => {
                        ...
                        return (
                            ...
                            >
                                <circle
                                    r={isGenre ? (isHovered ? 16 : 12) : (isHovered ? 8 : 5)}
                                    className={`${isGenre ? 'fill-accent' : 'fill-white/70'} shadow-xl transition-all duration-300`}
                                    filter={isGenre && isHovered ? 'url(#glow-genre)' : ''}
                                />
                                {(isGenre || isHovered || isConnected) && (
                                    <text
                                        dy={isGenre ? 32 : 20}
                                        textAnchor="middle"
                                        className={`text-[10px] pointer-events-none font-medium transition-all duration-300 ${isGenre ? (isHovered ? 'fill-accent scale-110' : 'fill-content-primary') : 'fill-content-secondary'}`}
                                    >
                                        {node.label}
                                    </text>
                                )}
                    ...
                    <div className="absolute bottom-6 right-6 p-4 bg-surface/80 backdrop-blur-md border border-white/10 rounded-2xl animate-in fade-in slide-in-from-bottom-2 max-w-xs transition-all">
                        {nodes[hoveredNode].type === 'genre' ? (
                            ...
                            <>
                                ...
                                <div className="mt-2 space-y-1">
                                    <p className="text-xs text-content-secondary">
                                        Genres: {links.filter(l => l.target === hoveredNode).map(l => nodes[l.source].label).join(', ')}
                                    </p>
                                    {nodes[hoveredNode].data.original_year && (
                                        <p className="text-[10px] text-content-muted">
                                            Year: {nodes[hoveredNode].data.original_year}
                                        </p>
                                    )}
                                </div>
                            </>
