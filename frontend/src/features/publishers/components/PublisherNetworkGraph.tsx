...
interface PublisherNetworkGraphProps {
    data: PublisherWithBooks[]
}

export function PublisherNetworkGraph({ data }: PublisherNetworkGraphProps) {
    ...
    useEffect(() => {
        ...
        // Create Nodes
        data.forEach(pubWithBooks => {
            const pubIndex = nodesList.length
            nodesList.push({
                id: `pub-${pubWithBooks.publisher.id}`,
                type: 'publisher',
                label: pubWithBooks.publisher.name,
                data: pubWithBooks.publisher,
            ...
            pubWithBooks.books.forEach(book => {
                ...
                linksList.push({ source: pubIndex, target: bookIndex })
            })
        })
    ...
    return (
        <div className="w-full bg-surface/20 rounded-3xl border border-white/5 overflow-hidden backdrop-blur-xl group">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <div>
                    <h3 className="text-lg font-semibold text-content-primary">Publisher & Book Map</h3>
                    <p className="text-sm text-content-muted">Publishers and their published works in an interactive network</p>
                </div>
                <div className="flex gap-4 text-xs">
                    <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full bg-accent shadow-[0_0_8px_rgba(226,161,140,0.4)]" />
                        <span className="text-content-secondary">Publisher</span>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <div className="relative h-[600px]">
                ...
                >
                    ...
                    {nodes.map((node, i) => {
                        ...
                        const isPub = node.type === 'publisher'
                        ...
                        return (
                            ...
                            >
                                <circle
                                    r={isPub ? (isHovered ? 16 : 12) : (isHovered ? 8 : 5)}
                                    className={`${isPub ? 'fill-accent' : 'fill-white/70'} shadow-xl transition-all duration-300`}
                                    filter={isPub && isHovered ? 'url(#glow-pub)' : ''}
                                />
                                {(isPub || isHovered || isConnected) && (
                                    <text
                                        dy={isPub ? 32 : 20}
                                        textAnchor="middle"
                                        className={`text-[10px] pointer-events-none font-medium transition-all duration-300 ${isPub ? (isHovered ? 'fill-accent scale-110' : 'fill-content-primary') : 'fill-content-secondary'}`}
                                    >
                                        {node.label}
                                    </text>
                                )}
                ...
                {hoveredNode !== null && nodes[hoveredNode] && (
                    <div className="absolute bottom-6 right-6 p-4 bg-surface/80 backdrop-blur-md border border-white/10 rounded-2xl animate-in fade-in slide-in-from-bottom-2 max-w-xs transition-all">
                        {nodes[hoveredNode].type === 'publisher' ? (
                            <>
                                <h4 className="font-bold text-accent">{nodes[hoveredNode].label}</h4>
                                <div className="mt-2 space-y-1">
                                    <p className="text-xs text-content-secondary">
                                        Published {links.filter(l => l.source === hoveredNode).length} books
                                    </p>
                                    <p className="text-[10px] text-content-muted italic">
                                        {nodes[hoveredNode].data.country || 'Global'}
                                    </p>
                                </div>
                            </>
                        ) : (
                            <>
                                ...
                                <div className="mt-2 space-y-1">
                                    <p className="text-xs text-content-secondary">
                                        Publisher: {links.filter(l => l.target === hoveredNode).map(l => nodes[l.source].label).join(', ')}
                                    </p>
                                    {nodes[hoveredNode].data.publish_year && (
                                        <p className="text-[10px] text-content-muted">
                                            Year: {nodes[hoveredNode].data.publish_year}
                                        </p>
                                    )}
                                </div>
                            </>
